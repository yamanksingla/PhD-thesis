...
%Machine learning approaches bring with them the culture of (training and) testing their models on large real-world datasets and pushing the state-of-the-art in terms of predictive accuracies. For instance, Facebook's click-through rate prediction models process billions of ad impressions daily, achieving prediction accuracies that directly translate to revenue optimization. Google's search ranking algorithms continuously predict user engagement with search results using hundreds of ranking factors, processing over 8.5 billion searches per day. However, often, ML approaches can only be operated as black boxes with no direct mechanism to explain predictions \cite{salganik2019bit,singla2022audio}. This creates a fundamental tension: while these systems can predict with remarkable accuracy that a particular ad will receive a 3.2\% click-through rate, they cannot explain \textit{why} in terms that human decision-makers can understand and act upon.
...
Our work makes several key contributions across four main areas:
...
Through this work, we aim to move beyond fragmented, platform-specific solutions toward a general understanding of human behavior that can be applied across diverse contexts, ultimately enabling more effective communication and content optimization.
